...
void config_parser::parse(const std::vector<uint8_t>& config_desc)
{
	const uint8_t* desc = &config_desc[0];
	int desc_len = static_cast<int>(config_desc.size());

    const usb_config_desc* header = reinterpret_cast<const usb_config_desc*>(desc);
    ...
    int offset = peek_desc_length(desc, 0);
    usb_alternate_interface* last_alternate = nullptr;

    while (offset + 2 < desc_len) {

        int len = peek_desc_length(desc, offset);
        usb_descriptor_type type = peek_desc_type(desc, offset);
        ...
        if (type == usb_descriptor_type::interface_association) {
            ...
            const usb_interface_association_desc* ia_desc = reinterpret_cast<const usb_interface_association_desc*>(desc + offset);
            ...
            const usb_interface_desc* intf_desc = reinterpret_cast<const usb_interface_desc*>(desc + offset);
            ...
            const usb_endpoint_desc* ep_desc = reinterpret_cast<const usb_endpoint_desc*>(desc + offset);
